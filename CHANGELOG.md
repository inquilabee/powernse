...
- `powernse verify --hashes` / `NSEData.audit_manifest() -> list[ManifestIssue]`: re-hash every staged file against the `sha256` recorded per download in `manifest/downloads.jsonl` and report missing files or checksum drift
