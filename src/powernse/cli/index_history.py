"""``powernse index-history`` -- deep per-index history into the ``index_closes`` layout."""
...
DEFAULT_FROM = date.fromisoformat(INDEX_HISTORY_FROM_DATE)
...
def register_index_history_command(app: typer.Typer) -> None:
    ...
    def index_history_cmd(
        index: Annotated[list[str] | None, typer.Option("--index", help=INDEX_HELP)] = None,
        all_indices: Annotated[bool, typer.Option("--all", help=ALL_HELP)] = False,
        from_date: Annotated[
            date | None,
            typer.Option("--from", parser=parse_iso_date, help=f"Start date YYYY-MM-DD (default {DEFAULT_FROM})"),
        ] = None,
        to_date: Annotated[
            date | None,
            typer.Option(
                ...
                help=f"End date YYYY-MM-DD (default {DEFAULT_TO}, the day before the ind_close_all floor)",
            ),
        ] = None,
        ...
        """Backfill index history to inception (~1995) via NSE's historical API, staged into index_closes."""
        ...
        summary = downloader.download_range(from_date or DEFAULT_FROM, to_date or DEFAULT_TO, names)
